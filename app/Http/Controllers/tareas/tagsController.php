...
class tagsController extends Controller
{
    ...
    {          $user = Auth::user();
       ...
       $services = ServiceUser::all();
       $service = array();
        foreach($services as $index=>$value) {

            $user = Users::find($value->id_solicitante);
            $value->id_solicitante = $value->id_solicitante .'|'.$user->first_name.' '.$user->last_name;
            if ($value->rel_responsable == 'Empresa' && $value->responsable == $user['attributes']['company']) {
                $service[] = $value;
            } else if ($value->rel_responsable == 'Departamento' && $value->responsable == $user['attributes']['division'] &&  $value->empresa_resp == $user['attributes']['company']) {
                $service[] = $value;
            } else if ($value->rel_responsable == 'Subdepartamento' && $value->responsable == $user['attributes']['subdivision'] &&  $value->empresa_resp == $user['attributes']['company']) {
                $service[] = $value;
            } else if ($value->rel_responsable == 'Puesto' && $value->responsable == $user['attributes']['workstation'] &&  $value->empresa_resp == $user['attributes']['company']) {
                $service[] = $value;
            } else if ($value->rel_responsable == 'Usuario' && $value->responsable == $user['attributes']['id'] &&  $value->empresa_resp == $user['attributes']['company']) {
                $service[] = $value;
            }else if($value->rel_responsable == "Usuario") {
               
                $value->responsable = $value->responsable .'|'.$user['attributes']['first_name'].' '.$user['attributes']['last_name'];
                $service[]= $value;
            }
        }
        return view('tareas.listado')->with('datos',$service);
    }
